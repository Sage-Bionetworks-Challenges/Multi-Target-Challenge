library(synapser)
library(tidyverse)
evaluation_id <- 9606273L
upload_dir_id <- "syn18510788" #put the synapse id of the directory you want to upload the files to
...
syn_ids <- synapser::synGetSubmissions(evaluation_id, status = "VALIDATED") %>%
  synapser::as.list() %>%
  purrr::map_chr("id") %>%
  as.integer()
